...
namespace Florist.Infrastructure.Data
{
    public static class DataSeeder
    {
        public static async Task SeedAsync(IServiceProvider serviceProvider)
        {
            ...
            var catHoaKho = await context.Categories.FirstOrDefaultAsync(c => c.Slug == "hoa-kho");
            if (catHoaKho == null) { catHoaKho = new Category { Id = Guid.NewGuid(), Name = "Hoa Khô (Dry Flowers)", Slug = "hoa-kho", IsActive = true }; context.Categories.Add(catHoaKho); }
            
            var catBoHoa = await context.Categories.FirstOrDefaultAsync(c => c.Slug == "bo-hoa");
            if (catBoHoa == null) { catBoHoa = new Category { Id = Guid.NewGuid(), Name = "Bó Hoa", Slug = "bo-hoa", IsActive = true }; context.Categories.Add(catBoHoa); }
            
            var catGioHoa = await context.Categories.FirstOrDefaultAsync(c => c.Slug == "gio-hoa");
            if (catGioHoa == null) { catGioHoa = new Category { Id = Guid.NewGuid(), Name = "Giỏ Hoa", Slug = "gio-hoa", IsActive = true }; context.Categories.Add(catGioHoa); }
            ...
            for(int i = 0; i < 9; i++)
            {
                var p = productsToSeed[i];
                var existingProduct = await context.Products.Include(x => x.Images).FirstOrDefaultAsync(x => x.Slug == p.Slug);
                
                if (existingProduct == null)
                {
                    context.Products.Add(p);
                    context.ProductVariants.Add(new ProductVariant { Id = Guid.NewGuid(), ProductId = p.Id, SKU = "SKU-00" + (i+1), Price = 250000 + (i * 50000), Stock = 100, IsActive = true });
                    context.ProductImages.Add(new ProductImage { Id = Guid.NewGuid(), ProductId = p.Id, ImageUrl = "/images/" + images[i], IsPrimary = true });
                }
                else
                {
                    if (!existingProduct.Images.Any())
                    {
                        context.ProductImages.Add(new ProductImage { Id = Guid.NewGuid(), ProductId = existingProduct.Id, ImageUrl = "/images/" + images[i], IsPrimary = true });
                    }
                }
            }
